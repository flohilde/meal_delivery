...
class DynamicInsertionPolicy_combine(Policy):
    ...
    def evaluate_insertion_impact(self, vehicle_id, insertion_point, customer_id, restaurant_id, obs):
        ...
        # Calculate old delay and freshness
        old_delay = self.calculate_route_delay(vehicle_id, old_route, obs)
        old_freshness = self.calculate_route_freshness(vehicle_id, old_route, obs)
        # Creating a tentative route by deep copying and inserting new actions
        tentative_route = copy.deepcopy(old_route)

        # Calculate old combined impact
        old_combined_impact = old_delay + old_freshness
        ...
        tentative_freshness = self.calculate_route_freshness(vehicle_id, tentative_route, obs)

        # Calculate tentative combined impact
        tentative_combined_impact = tentative_delay + tentative_freshness

        # Calculate the impact as the difference in the combined impact
        return tentative_combined_impact - old_combined_impact
    ...
    def calculate_route_freshness(self, vehicle_id, route, obs):
        total_freshness = 0
        total_delivered_orders = 0

        for stop in route:
            if stop["type"] == "pickup":
                restaurant_id = stop["destination"]
                if restaurant_id in obs["restaurant_info"]:
                    restaurant_info = obs["restaurant_info"][restaurant_id]
                    for order_id in stop["orders_to_pickup"]:
                        
                        matching_order = next((order for order in obs["placed_orders"] if order.customer_id == order_id and order.restaurant_name == restaurant_id), None)
                        if matching_order and matching_order.finished_at is not None:
                            # Calculate freshness based on the finished_at attribute
                            delivery_stop = next((s for s in route if s["type"] == "delivery" and s["customer_id"] == order_id), None)
                            if delivery_stop:
                                delivery_time = delivery_stop["actual_arrival_time"] 
                                freshness = delivery_time - matching_order.finished_at
                                total_freshness += max(0, freshness)
                                total_delivered_orders += 1

        return (total_freshness / total_delivered_orders) if total_delivered_orders > 0 else 0
    ...
    def repair_route(self, route, obs):
        ...
        for stop_index, stop in enumerate(repaired_route):
            ...
            if isinstance(stop_dict, Stop) and stop_dict.type == "pickup":
                ...
                repaired_route[stop_index] = Stop(**stop_dict)  # Convert the dictionary back to Stop namedtuple
